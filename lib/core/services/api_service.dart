...
class ApiService {
  ...
  // ── Auth headers WITHOUT Content-Type (for multipart / Dio FormData) ──
  // When sending FormData, Dio MUST set the Content-Type itself so it can
  // include the correct boundary. Never override it manually.
  ...
  static Future<void> logout() async {
    try {
      await _dio.post('/api/v1/sync/logout', options: await _authOpts());
    } catch (_) {}
    await clearSession();
  }
  ...
  static Future<Map<String, dynamic>?> getProfile() async {
    try {
      final res = await _dio.get('/api/v1/sync/profile',
          options: await _authOpts());
      final prof = Map<String, dynamic>.from(
          (res.data as Map)['profile'] as Map);
      final token = await getToken() ?? '';
  ...
  /// Search business by name — bumps search_count on server.
  ...
  static Future<List<Map<String, dynamic>>> getMyBusinesses() async {
    try {
      final res = await _dio.get('/api/v1/sync/businesses',
          options: await _authOpts());
      return List<Map<String, dynamic>>.from(
          ((res.data as Map)['businesses'] as List)
              .map((e) => Map<String, dynamic>.from(e as Map)));
  ...
  // ── Add business (with optional image upload) ────────────────
  // IMPORTANT: When sending FormData (multipart), do NOT manually set
  // Content-Type. Dio adds the correct value + boundary automatically.
  static Future<Map<String, dynamic>> addBusiness({
    ...
    try {
      if (imageFile != null) {
        ...
        // Do NOT set Content-Type — Dio adds multipart/form-data; boundary=xxx
        final opts = await _authOptsMultipart();
  ...
  static Future<List<Map<String, dynamic>>> getBookmarks() async {
    try {
      final res = await _dio.get('/api/v1/sync/bookmarks',
          options: await _authOpts());
      return List<Map<String, dynamic>>.from(
  ...
  static Future<bool> syncBookmarks(
