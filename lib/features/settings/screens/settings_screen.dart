...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  VoidCallback _confirm(
      String title, String body, Future<void> Function() fn) {
    return () async {
  ...
  // ── avatarColor: stored as string ('cyan','red','orange','purple','green')
  Color _resolveAvatarColor() {
    final raw = _profile['avatarColor'];
    // Handle both legacy int index and new string format
    if (raw is int) {
      final colors = [
        ...
        children: [
          ...
          _card([
            ...
            _actionTile(
              ...
              _confirm('Clear Downloads',
                  'Remove all download records?', () async {
                await LocalDB.clearDownloads();
                _snack('Downloads cleared');
              }),
            ),
            _actionTile(
              Icons.manage_search_rounded, 'Clear Search History',
              'Remove recent searches', AppTheme.accentCyan,
              _confirm('Clear Search History',
                  'Remove all recent searches?', () async {
